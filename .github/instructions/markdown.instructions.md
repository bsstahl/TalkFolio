...
* Use YAML frontmatter only when a file benefits from machine-readable metadata such as a document title or explicit summary.
* Keep frontmatter minimal and consistent with the surrounding repo conventions.
* When frontmatter is used, keep the first content in the file after the closing `---` and do not duplicate the title in the body if it is already present in metadata.
